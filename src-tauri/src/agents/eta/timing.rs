//! Timing calculations for ETA estimation.

use chrono::{DateTime, Utc};
use std::collections::HashMap;
use std::sync::Arc;

use crate::db::{Database, EtaConfidence};
...
pub fn calculate_timing_stats(
    ...
    let child_ticket_ids: Vec<_> = tickets
        .iter()
        .filter(|t| !t.is_epic)
        .map(|t| t.id.clone())
        .collect();
                ...
                if let Some(ended) = run.ended_at {
                    ...
                    if duration > 0.0 {
                        ...
                        if let Some(stage) = run.stage {
                            stage_durations.entry(stage).or_default().push(duration);
                        }
                    }
                }
            }
    ...
    let avg_per_stage: HashMap<String, f64> = stage_durations
        .into_iter()
        .map(|(stage, durations)| {
...
pub fn calculate_remaining_time(
    ...
    let estimated_remaining =
        (not_started as f64 * avg) + (in_progress as f64 * avg / 2.0) + (paused as f64 * avg);
