...
//!
//! Delegates to the shared `command_templates` module with ".cursor" as the
//! config directory.
...
pub fn check_project_commands_installed(repo_path: &Path) -> bool {
    command_templates::check_project_commands_installed(repo_path, CONFIG_DIR)
}
...
pub fn install_commands(repo_path: &Path, commands_source: &Path) -> std::io::Result<Vec<String>> {
    command_templates::install_commands(repo_path, CONFIG_DIR, commands_source)
}
